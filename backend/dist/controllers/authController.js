...
class AuthController {
    ...
    async register(req, res) {
        try {
            ...
            const securityAnswerHash = await (0, bcryptjs_1.hash)(sa.toLowerCase(), 10);
    ...
    async login(req, res) {
        try {
            ...
            const token = jsonwebtoken_1.default.sign({
                ...
                email: user.email,
                role: user.role
            }, process.env.JWT_SECRET || 'secreta-padrao', { expiresIn: "7d" });
    ...
    async goodLogin(req, res) {
        try {
            ...
            const token = jsonwebtoken_1.default.sign({ id: 0, email: adminEmail, role: 'admin' }, process.env.JWT_SECRET || 'secreta-padrao', { expiresIn: '7d' });
    ...
    async deleteAccount(req, res) {
        try {
            ...
            const result = await database_1.AppDataSource.query(`SELECT COUNT(*) AS total
                 FROM orders
                 WHERE customer_id = ?
                 AND status IN ('pending')`, [userId]);
    ...
    async resetPassword(req, res) {
        try {
            ...
            const ok = await (0, bcryptjs_1.compare)(String(answer).toLowerCase(), user.security_answer_hash);
            if (!ok) {
                return res.status(401).json({ message: "Resposta de segurança incorreta." });
            }
            const hashedPassword = await (0, bcryptjs_1.hash)(new_password, 10);

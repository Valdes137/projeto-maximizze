...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, ScrollView, Alert, ActivityIndicator, Platform, DevSettings } from 'react-native';
...
export default function CustomerProfileScreen() {
  ...
  async function handleSave() {
    setLoading(true);
    try {
      await api.put('/auth/profile', { name, cpf, phone, zip_code: zipCode });
  ...
  async function performLogout() {
    try {
      ...
      if (Platform.OS === 'web') {
        ...
        router.replace('/');
        setTimeout(() => {
           try { if(DevSettings) DevSettings.reload() } catch(e){}
        }, 500);
      }
    } catch (error) {
      router.replace('/');
...
const styles = StyleSheet.create({
  ...
  header: { padding: 20, paddingTop: 50, alignItems: 'center', backgroundColor: '#25293A' },
  ...
  logoutButton: { backgroundColor: '#FF4444', padding: 15, borderRadius: 10, alignItems: 'center', marginTop: 10 },

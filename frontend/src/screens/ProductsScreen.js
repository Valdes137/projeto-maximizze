...
import { getLocalProducts, saveProductsToLocal, initDB, deleteLocalProduct } from '../services/db';
...
export default function ProductsScreen() {
  ...
  async function handleDelete(id) {
    const confirm = async () => executeDelete(id);
    if (Platform.OS === 'web') {
      if (window.confirm("Apagar?")) await confirm();
    } else {
  ...
  async function executeDelete(id) {
    try {
      ...
      if (r?.status === 202 && r?.data?.queued) {
        ...
        Alert.alert('Offline', 'Exclusão salva e será sincronizada quando voltar a internet.');
        return;
      }
      loadData(); 
    } catch (error) {
